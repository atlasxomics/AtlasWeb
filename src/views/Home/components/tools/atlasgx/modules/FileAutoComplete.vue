  ...
  setup(props, ctx) {
    ...
    async function querySelections(v: string) {
      ...
      setTimeout(async () => {
        ...
        filteredOnes.value = candidates.value.filter((g: any) => g.name.toLowerCase().includes(v.toLowerCase()) || selectedOnes.value === g.name);
    ...
    async function onChanged(ev: any) {
      ...
      // ctx.emit('changed', ev);
      // console.log('changed');
      // console.log(`emit changed ${selectedOnes.value}`);
