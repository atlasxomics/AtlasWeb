<template>
  <v-card>
    <v-card-title class="text-capitalize">
      Upload DBiT Information File (.csv)
    </v-card-title>
    <v-card-text>
      <v-file-input
        v-model="files"
...
import { uploadDbitInformationMenu } from '../state';
import { files } from '../../state';

export default defineComponent({
  name: 'DbitInformationUpload',
  setup() {
    const client = computed(() => store.state.client);
    const metaType = ref<string>('dbits');
    ...
    function closeDialog() {
      uploadDbitInformationMenu.value = false;
      files.value = [];
    }
